...
export function WizardStepper({ steps, currentStep, setStep }: WizardStepperProps) {
  return (
    <div className="flex justify-between items-center mb-10 relative w-full max-w-4xl mx-auto print:hidden pt-8 px-4">
      <div className="absolute top-1/2 left-4 right-4 h-1.5 bg-gray-200 -z-10 -translate-y-1/2 rounded-full"></div>
      <div
        className="absolute top-1/2 left-4 h-1.5 bg-brand-light -z-10 -translate-y-1/2 transition-all duration-500 rounded-full"
      ...
      {steps.map((s, i) => {
        ...
        return (
          ...
          >
            ...
            <span
              className={cn(
                'text-[10px] sm:text-xs font-medium absolute -bottom-7 whitespace-nowrap transition-colors duration-300',
                isActive
