      ...
      <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
        <Table>
          ...
          <TableBody>
            ...
            {logs.map((log) => (
              <TableRow key={log.id}>
                <TableCell className="text-xs text-muted-foreground">{new Date(log.timestamp).toLocaleString('pt-BR')}</TableCell>
                ...
                <TableCell className="text-xs text-gray-500 max-w-[200px] truncate" title={log.newData || log.previousData}>
                  {log.newData ? `Novo: ${log.newData}` : log.previousData ? `Antigo: ${log.previousData}` : 'N/A'}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
